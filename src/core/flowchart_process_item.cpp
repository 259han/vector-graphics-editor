...
QRectF FlowchartProcessItem::boundingRect() const
{
    // 返回图元的边界矩形
    return QRectF(-m_size.width() / 2, -m_size.height() / 2, m_size.width(), m_size.height());
}

void FlowchartProcessItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    // 使用缓存时调用基类方法
...
QPainterPath FlowchartProcessItem::shape() const
{
    // 创建图形路径
    QPainterPath path;
...
// 重写contains方法，实现更准确的矩形内部检测
bool FlowchartProcessItem::contains(const QPointF& point) const
{
    // 将点从场景坐标转换为图形项的本地坐标
    ...
    // 创建一个扩展的矩形区域用于检测
    QPainterPathStroker stroker;
    qreal extraTolerance = 1500.0; // 设置为固定的1500，比之前的1000更大
...
QPainterPath FlowchartProcessItem::toPath() const
{
    // 返回图形的路径表示，用于裁剪等操作
    return shape();
}

std::vector<QPointF> FlowchartProcessItem::getDrawPoints() const
{
    // 返回图形的顶点，用于序列化等操作
